 ...
 * Main CPU code.
 *
 *  TODO: Uncomment the code the uses CUDA 3D memory and make it work.
 *
 */
...
extern "C"
void computeGold(cudaPitchedPtr *reference, float *idata, size_t ntimepts, size_t width, size_t height);

extern "C" 
void executeCudaKernel(size_t threadsPerBlock, size_t blocksPerGrid, cudaPitchedPtr *g_idata, float *g_odata, size_t ntimepts, size_t width, size_t height);
...
runTest(int argc, char **argv)
{
    ...
    cudaExtent extent = make_cudaExtent(ntimepts*sizeof(float), width, height);  // in bytes or floats? cudaMalloc3D below does not know it's float, so must be in bytes.
...
//    checkCudaErrors(cudaMemcpy(h_odata, d_odata, image_size, cudaMemcpyDeviceToHost));   // TODO Fails here at runtime!!!! Strange as this code is the same as linear mem case.
    ...
    free(h_odata);
    free(reference);
	checkCudaErrors(cudaFree(d_idata.ptr));  // TODO is this the way to free device 3D mem? - NO it fails here!!!!
